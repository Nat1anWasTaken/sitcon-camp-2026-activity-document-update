## SITCON Camp 2026 課活組文件索引

## 長期、籌會

## 長期

- [課活組工具使用指南](/M11bsNAISbe_GaK4hLrp9Q)
- [課活組營期課表](https://docs.google.com/spreadsheets/d/1D887C9Gzc14p5y5vUnoitAX6Y5eWH0nsXLdRhZhcntY/edit)

## 籌會

- [課活組零籌](/cR6DSkI1TsenjCeM0wTp4g)
- [課活組零點五籌（主線課程講者表）](https://docs.google.com/spreadsheets/d/1FwevZG_33k4FGfsJKfWvjiETWyLM-w1z4iiac_IVeq4/edit?gid=0#gid=0)
- [課活組零點五籌（主線課程講者討論）](/H1IIkdVp-x)
- [課活組一籌](/9u0VtThVRKqfitx3lY0Bfg)
- [課活組二籌](/AxFkk4ByQ4empP0nQqdzdQ)
- [課活組三籌](/-1Ul0d2cRu2DM9AC4zu9gw)
- [課活組四籌](/yjbSOzjISBeUTH8ebOerSg)
- [課活組五籌](/THysX6FpSEinm5jLmzHEYg)
- [課活組六籌](/4sl9Vab1QQaeBjEgebRB_A)
- [課活組七籌](/BJsVrQuxMe)

## 主線課程

- [ML 課程主軸對齊](/sSw4-Y0yT8q6JSjiPfFSXg)

## 各課程活動、點數系統

## 實境解謎 (Day 4)

- [實境解謎同步會議](/BJwxdE6yfl)
- [海森堡 bug 交接會議](/ryhPeewZMx)

## 闖關活動 (Day 1)

- [闖關活動同步會議](/HyUeC7hlzx)

## 讓你的 agent 上場戰鬥吧

- [讓你的 agent 上場戰鬥吧](/HkoPqfPxMe)

## 視界咖啡館

- [視界咖啡館初步筆記](/rymsPJygzl)

## 破解位元城的都市傳說

- [破解位元城的都市傳說](https://docs.google.com/document/d/1T8RnAOHptevv-s02uU9PRAPVXAlRvBTJBRhYS9Xo2CU/edit?usp=sharing)

## 點數系統

- [點數系統設計方向](/S1zeqB6kMg)
- [點數系統規則評估與調整建議](/BJxjRnfxfe)

## SIGs 爐邊夜談

- [SIGs 爐邊夜談](/HkegLkuezl)
- [爐邊夜談開桌登記](/ryjWvelWMx)
- [爐邊夜談（SIGs）](/B1eTC8LZfg)

## 其他

## 其他討論

- [相見歡流程討論](/rk-HoAIJfx)
- [交大教室借用表](https://docs.google.com/spreadsheets/d/1VLaObDnB51FHQeDopEH_vyni8Kt97sWfsatRzwHmZ10/edit?usp=sharing)

## 招募

- [課活組面談小規劃（棄用）](/kLFnh0TZRr6yc52sqHnNGA)

<style>
.summary #summary > h2:not(.collapsible):not(:first-child) {
  margin: 20px 0 12px !important;
  padding: 0 !important;
}

.summary #summary > h2:not(.collapsible):not(:first-child) {
  display: flex !important;
  align-items: center;
  gap: 10px;
  width: 100%;
  height: auto;
  padding: 0 !important;
  overflow: hidden;
  background: transparent !important;
  color: inherit !important;
  opacity: 0.72;
  font-size: 12px;
  line-height: 1;
  font-weight: 600;
}

.summary #summary > h2:not(.collapsible):not(:first-child)::before,
.summary #summary > h2:not(.collapsible):not(:first-child)::after {
  content: "";
  display: block;
  height: 1px;
  min-width: 12px;
  flex: 1 1 auto;
  background: currentColor;
  opacity: 0.34;
}
</style>
